...
const PaintBannerComponent = ({ paint, navigation }) => {
  let count = 0;
  let isWorked = false;
          ...
          <View style={styles.bottomViews}>
            <SpaceBetween>
              <TouchableOpacity
                onPress={() => {
                  ...
                  setTimeout(async () => {
                    if (count > 1 && !isWorked) {
                      await addRunCountService(paint?._id);
                      await gameLinkMaker(paint, navigation);

                      isWorked = true;
                    } else if (count <= 1 && !isWorked) {
                      navigation.navigate("Game Post", { id: paint._id });
                      isWorked = true;
                    }
                  }, 300);
                  setTimeout(() => {
              ...
              <View>
                <CustomText fontSize={18}>{paint?.title||"برترین نقاشی"}</CustomText>
            ...
            <SpaceStyle top={10}>
              <Row>
                ...
                <View style={styles.secondBadge}>
                  <Row>
                    <CustomText >{paint?.userScoreCount || 100}</CustomText>
                    <Icon
                ...
                <View style={styles.secondBadge}>
                  <Row>
                    <CustomText selfCenter>{paint?.downloadCount || 10}</CustomText>
                    <Icon
                      style={{ width: 12, height: 12, marginTop: 2, marginLeft: 5 }}
...
export default PaintBannerComponent;
