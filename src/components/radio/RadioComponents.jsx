...
import RadioComponentItem from "./RadioComponentItem";

const RadioComponents = ({ title, radios, navigation, sort = "_id", cid = "" }) => {
  return (
    <SpaceStyle top={20}>
      <SpaceBetween>
        <SpaceStyle left={5} right={5} bottom={15}>
          <TouchableOpacity onPress={() => navigation.navigate("Public Page", { data: radios, mode: "Voice PlayList", title, sort, cid })}>
            <Row>
              <Icon
      ...
      <FlatList
        ...
        data={radios}
        renderItem={({ item }) => <RadioComponentItem item={item} navigation={navigation} />}
      />
    </SpaceStyle>
  );
};
