...
const UserPosts = () => {
  ...
  const updatePosts = async()=>{
    ...
    setIsLoading(true)
    await axios.get("https://lifelands.ir/api/v1/social/userpost",{
      headers:{
        token: getValueFor()
      }
    }).then(res=>{
  ...
  const deletePost = async(post)=>{
    console.log(post.postId._id)
    await axios.post('https://lifelands.ir/api/v1/social/deletePost',{
  ...
  return (
    ...
    >
      ...
      <NavbarSh rightChile={<View style={{flexDirection:'row',alignItems:'center'}}>
        <Text style={{color:'white',fontFamily:'vazir',marginRight: 10}}>پست ها</Text>
        <GoBack />
      </View>}/>
     {
        ...
        <FlatList
          ...
          if(item.postId){
            return (
              ...
              >
                <TouchableOpacity style={{position:"absolute",top:8,right: 8,zIndex:999}} onPress={()=>{
                    deletePost(item)
                }}>
                <FontAwesome6 name="delete-left" color={'#ff003c'} size={RFPercentage(3)} />
